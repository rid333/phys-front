...
<div class="relative flex items-center justify-center">
    <h1
        class="text-5xl lg:text-7xl absolute text-white font-bold z-10 text-center"
    >
        {pageName}
    </h1>
    <img
    ...
    <div class="-mt-20 bg-white mb-10 shadow-lg p-8 lg:p-16">
        ...
        <Accordion.Root class={`mt-10 p-2.5 lg:p-5 text-white bg-${color}`}>
            <Accordion.Item value="item-1">
                <Accordion.Trigger class="text-sm lg:text-xl font-bold"
                    >The thesis titles of students in the {pageName}</Accordion.Trigger
                >
        ...
        <div class="grid lg:grid-cols-2 gap-10">
            {#each dosen as item}
                <a href={`/people/${item.id}`} class="flex gap-5">
                    <img
                        class="w-28 h-28 lg:w-28 lg:h-32 aspect-square rounded-none"
                        src={`${PUBLIC_API_URL}${item.photo.url}`}
                        alt={item.name}
                    />
                    <div class="flex flex-col">
                        <div
                            class={`font-semibold text-lg text-white bg-${color} p-2 mb-2 w-fit`}
                        >
                            {item.name}
                        </div>
                        <div class="text-base font-medium">{item.position}</div>
                        <div
                            class="text-base font-medium flex items-center gap-x-0.5"
                        >
                            <Send size="16px" strokeWidth="1px" />{item.email}
                        </div>
                    </div>
                </a>
            {/each}
