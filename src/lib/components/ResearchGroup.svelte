...
<div class="max-w-5xl m-auto relative">
    <div class="-mt-20 bg-white mb-10 shadow-lg p-10 lg:p-16">
        ...
        <p class="max-w-full text-black text-lg lg:text-xl font-medium lg:leading-relaxed text-justify prose">
            <Serializer richText={content} />
        </p>
        <Accordion.Root class={`mt-10 p-5 text-white bg-${color}`}>
            <Accordion.Item value="item-1">
            <Accordion.Trigger class="text-base lg:text-xl font-bold">The thesis titles of students in the Theoretical and Computational Physics</Accordion.Trigger>
            {#each thesis as judul}
                <Accordion.Content>
                    <p class="text-base lg:text-lg font-medium text-justify">
                        {@html judul}
                    </p>
                </Accordion.Content>
            {/each}
    ...
    <div class="bg-white p-16 mb-20 shadow-xl">
        <h1 class={`text-${color} text-3xl font-bold mb-10`}>Academic Staff</h1>
        <div class="grid lg:grid-cols-2 gap-10">
            {#each dosen as item}
                <a href={`/people/${item.id}`} class="flex gap-8">
                    <img class="w-28 h-28 lg:w-32 lg:h-32 aspect-square rounded-none" src={`http://localhost:3000${item.photo.url}`} alt={item.name}/>
